...
export function MemoriaEditor({ sandboxId, basePath, memoriaData, onBack, confirmationService }) {
  ...
  const handleAddStream = async () => {
    ...
    const newStream = { config: {}, entries: [] };
    setLoading(true);
    setErrorMessage('');
    try {
        await mutate(sandboxId, [{ type: 'UPSERT', path: `${basePath}/${name}`, value: newStream }]);
        setStreams(prev => ({ ...prev, [name]: newStream }));
        setNewStreamName('');
        setExpandedStreams(prev => ({ ...prev, [name]: true }));
    } catch(e) {
        setErrorMessage(`Failed to add stream: ${e.message}`);
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleDeleteStream = async (streamName) => {
    const confirmed = await confirmationService.confirm({
      title: '删除流确认',
      message: `你确定要删除流 "${streamName}" 吗？此操作无法撤销。`,
    });
    if (!confirmed) return;
    setLoading(true);
    setErrorMessage('');
    try {
        await mutate(sandboxId, [{ type: 'DELETE', path: `${basePath}/${streamName}` }]);
        setStreams(prev => {
  ...
  const handleAddEntry = async (streamName) => {
    const nextSequenceId = globalSequence + 1;
    const newEntry = {
      id: crypto.randomUUID(), sequence_id: nextSequenceId, content: '新记忆条目，请编辑内容。',
      level: 'event', tags: [], created_at: new Date().toISOString(),
      metadata: { source: 'sandbox-editor' }, _internal_id: `entry_internal_${Date.now()}`
    };
    const updatedEntries = [...streams[streamName].entries, newEntry];
    const cleanEntriesToSave = updatedEntries.map(({ _internal_id, ...rest }) => rest);
    setLoading(true);
    setErrorMessage('');
    try {
        await mutate(sandboxId, [
            { type: 'UPSERT', path: `${basePath}/${streamName}/entries`, value: cleanEntriesToSave },
            { type: 'UPSERT', path: `${basePath}/__global_sequence__`, value: nextSequenceId }
        ]);
        setStreams(prev => ({ ...prev, [streamName]: { ...prev[streamName], entries: updatedEntries } }));
        setGlobalSequence(nextSequenceId);
        setExpandedEntries(exp => ({ ...exp, [newEntry._internal_id]: true }));
    } catch (e) {
        setErrorMessage(`Failed to add entry: ${e.message}`);
    } finally {
        setLoading(false);
    }
  };

  const handleDeleteEntry = async (streamName, entryInternalId) => {
    const updatedEntries = streams[streamName].entries.filter(e => e._internal_id !== entryInternalId);
    const cleanEntriesToSave = updatedEntries.map(({ _internal_id, ...rest }) => rest);
    setLoading(true);
    setErrorMessage('');
    try {
        await mutate(sandboxId, [{ type: 'UPSERT', path: `${basePath}/${streamName}/entries`, value: cleanEntriesToSave }]);
        setStreams(prev => ({ ...prev, [streamName]: { ...prev[streamName], entries: updatedEntries } }));
    } catch (e) {
        setErrorMessage(`Failed to delete entry: ${e.message}`);
    } finally {
        setLoading(false);
    }
  };
  
  const debouncedEntrySave = useCallback(debounce(async (streamName, updatedEntries) => {
      const cleanEntriesToSave = updatedEntries.map(({ _internal_id, ...rest }) => rest);
      setLoading(true);
      setErrorMessage('');
      try {
          ...
          setErrorMessage(`Auto-save failed: ${e.message}`);
      } finally {
          setLoading(false);
      }
  ...
  const handleEntryChange = (streamName, entryInternalId, field, value) => {
    ...
    if (field === 'tags' && typeof value === 'string') finalValue = value.split(',').map(t => t.trim().toLowerCase()).filter(Boolean);
    if (field === 'sequence_id') finalValue = parseInt(value, 10) || 0;
    
    const updatedEntries = streams[streamName].entries.map(entry => 
      entry._internal_id === entryInternalId ? { ...entry, [field]: finalValue } : entry
    );
    setStreams(prev => ({ ...prev, [streamName]: { ...prev[streamName], entries: updatedEntries } }));
    debouncedEntrySave(streamName, updatedEntries);
  };
  ...
  const handleEntryDragEnd = async (streamName, event) => {
    ...
    if (active && over && active.id !== over.id) {
      ...
      const cleanEntriesToSave = reorderedEntries.map(({ _internal_id, ...rest }) => rest);
      setLoading(true);
      setErrorMessage('');
      try {
          await mutate(sandboxId, [{ type: 'UPSERT', path: `${basePath}/${streamName}/entries`, value: cleanEntriesToSave }]);
          ...
          setErrorMessage(`Failed to reorder entries: ${e.message}`);
      } finally {
          setLoading(false);
      }
    }
  ...
  const toggleStreamExpand = (streamName) => setExpandedStreams(prev => ({ ...prev, [streamName]: !prev[streamName] }));
  const toggleEntryExpand = (entryInternalId) => setExpandedEntries(prev => ({...prev, [entryInternalId]: !prev[entryInternalId] }));
  ...
  return (
    <Box sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
        ...
        <Button variant="outlined" startIcon={<AddIcon />} onClick={handleAddStream} disabled={loading}>添加Stream</Button>
      </Box>
        ...
        <List disablePadding>
          {Object.entries(streams).map(([streamName, streamData]) => (
            <React.Fragment key={streamName}>
              <ListItem button onClick={() => toggleStreamExpand(streamName)} sx={{ borderBottom: '1px solid rgba(255, 255, 255, 0.12)' }}>
                ...
                <IconButton edge="end" onClick={(e) => { e.stopPropagation(); handleDeleteStream(streamName); }} disabled={loading}>
                  <DeleteIcon />
                </IconButton>
              </ListItem>
              <Collapse in={!!expandedStreams[streamName]} timeout="auto" unmountOnExit>
                <Box sx={{ pl: 4, pr: 2, pb: 2, pt: 1 }}>
                  <Button variant="outlined" startIcon={<AddIcon />} onClick={() => handleAddEntry(streamName)} size="small" sx={{ mb: 2 }} disabled={loading}>添加条目</Button>
                      ...
                      <List disablePadding>
                        {(streamData.entries || []).map((entry) => (
                          <SortableMemoryEntryItem
                            key={entry._internal_id} id={entry._internal_id} entry={entry}
