...
def setup_test_app_state():
    """
    【全新 Session-wide Fixture】
    在整个测试会话开始时，模拟 main.py 的 setup_application() 流程。
    - 动态加载所有模块，填充所有注册表。
    - 创建一个包含 mock 服务的服务总线。
    - 将配置好的引擎实例附加到 app.state 上。
    
    `autouse=True` 确保它在所有测试之前自动运行。
    """
    print("\n--- Setting up Test Application State for the session ---")
    
    # 1. 定义可插拔模块 (与 main.py 保持一致)
    PLUGGABLE_MODULES = [
        "backend.runtimes",
        "backend.llm.providers",
        # "backend.services"
    ]
    # 在 backend/services 目录下创建一个空的 __init__.py 以使其成为一个包
    # 并将 llm/service.py 移动到 services/llm_service.py 
    # (假设您已采纳这个结构建议)
    
    # 2. 动态加载模块，填充所有注册表
    load_modules(PLUGGABLE_MODULES)
    
    # 3. 总是使用 Mock 服务来构建测试服务总线
    mock_services = {
        "llm": MockLLMService()
    }

    # 4. 实例化测试用的引擎
    # 注意：我们不再需要 populated_registry fixture，因为 runtime_registry 已经被自动填充
    test_engine_instance = ExecutionEngine(
        registry=runtime_registry,
        services=mock_services
    )
    
    # 5. 将引擎实例附加到 FastAPI app 的 state 上
    # 这使得 E2E API 测试能够使用与单元/集成测试完全相同的引擎实例
    app.state.engine = test_engine_instance
    print("--- Test Application State setup complete ---")
...
def test_engine() -> ExecutionEngine:
    """
    【极大简化】这个 fixture 现在只负责从 app.state 中返回已配置好的引擎实例。
    """
...
def test_client() -> Generator[TestClient, None, None]:
    """
    【简化】这个 fixture 不再需要自己配置 app，因为它已经在 session setup 中完成了。
    它只负责清理存储。
    """
    sandbox_store.clear()
    snapshot_store.clear()
    
    # TestClient 会使用在模块级别定义的全局 app 对象
...
class TestEngineSubgraphExecution:
    ...
    async def test_subgraph_failure_propagates_to_caller(self, test_engine, subgraph_with_failure_collection: GraphCollection):
        ...
        assert isinstance(downstream_output, dict)
