...
2.  **指令前求值**: 在每次循环中，它首先调用 `evaluation.evaluate_data` 来处理当前指令的 `config`。`build_evaluation_context` 会确保此时的 `world`, `nodes`, `pipe` 等上下文都是最新的。
3.  **运行时纯粹性**: 运行时接收到的 `config` 是纯粹、已求值的 Python 对象。这极大地简化了运行时的设计和测试。
