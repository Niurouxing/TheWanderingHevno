...
2.  [核心概念与设计哲学](#2-核心概念与设计哲学)
    *   [2.1 愿景：从“聊天机器人”到“世界模拟器”](#21-愿景从聊天机器人到世界模拟器)
    *   [2.2 图执行哲学](#22-图执行哲学)
3.  [REST API 参考](#3-rest-api-参考)
    *   [3.1 系统与插件 API (`/api/system`, `/api/plugins`)](#31-系统与插件-api-apisystem-apiplugins)
    *   [3.2 沙盒 API (`/api/sandboxes`)](#32-沙盒-api-apisandboxes)
    *   [3.3 持久化 API (`/api/persistence`)](#33-持久化-api-apipersistence)
4.  [WebSocket API 参考 (`/ws/hooks`)](#4-websocket-api-参考-wshooks)
    *   [4.1 连接](#41-连接)
    *   [4.2 客户端到服务端消息](#42-客户端到服务端消息)
    *   [4.3 服务端到客户端消息](#43-服务端到客户端消息)
5.  [核心数据模型参考](#5-核心数据模型参考)
    *   [5.1 `Sandbox`](#51-sandbox)
    *   [5.2 `StateSnapshot`](#52-statesnapshot)
    *   [5.3 `GraphCollection` 与 `GraphDefinition`](#53-graphcollection-与-graphdefinition)
    *   [5.4 `GenericNode` 与 `RuntimeInstruction`](#54-genericnode-与-runtimeinstruction)
    *   [5.5 `Memoria` 记忆系统模型](#55-memoria-记忆系统模型)
6.  [图与宏系统参考](#6-图与宏系统参考)
    *   [6.1 图定义格式](#61-图定义格式)
    *   [6.2 宏系统 (`{{...}}`)](#62-宏系统-)
    *   [6.3 `system` 运行时参考](#63-system-运行时参考)
...
*   **摘要**: 获取前端插件清单。
*   **描述**: 返回 `hevno.json` 中定义的所有 `type: "frontend"` 的插件配置。这是前端动态加载插件的**唯一事实来源**。
*   **响应 (200 OK)**:
    ```json
    [
      {
...
*   **请求体**: `application/json`
    ...
    ```
*   **响应 (201 Created)**: 返回创建的 `Sandbox` 对象。
    ```json
    {
      "id": "a1b2c3d4-e5f6-...",
      "name": "我的第一个互动小说",
      "head_snapshot_id": "f6e5d4c3-b2a1-...",
...
*   **摘要**: 在沙盒的最新状态上执行一步计算。
*   **描述**: 这是驱动世界演化的核心端点。它接收用户输入，在当前 `head_snapshot_id` 指向的快照上执行图，并生成一个新的快照。
...
*   **请求体**: `application/json` (用户输入，将注入到宏的 `run.trigger_input` 上下文中)。
    ...
    ```
*   **响应 (200 OK)**: 返回新生成的 `StateSnapshot` 对象。
*   **错误**:
    *   `404 Not Found`: 沙盒未找到。
...
*   **摘要**: 列出指定类型的所有已保存资产。
*   **描述**: 用于获取服务器上存储的图、Codex等资源的列表。
*   **路径参数**: `asset_type` (Enum: `"graph"`, `"codex"`)
*   **响应 (200 OK)**: 资产名称字符串数组。
    ```json
    [
      "main_story_flow",
